 ...
 * local backend (server/), which holds the Service Account credential.
 * The backend matches the row by name+phone and updates it in place — it
 * never appends a new row.
 */
export async function updateCsMasterLatLng(name: string, phone: string, lat: number, lng: number): Promise<void> {
  const res = await fetch(`${API_BASE}/api/cs-master/update-location`, {
    method: 'POST',
